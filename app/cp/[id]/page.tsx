...
export default function CabinetProposalPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const lk = estimate?.status === 'approved'
    ? (estimate as CabinetEstimateRecord & {
  ...
  let subtotal: number, discountAmount: number, taxAmount: number,
  ...
  if (isLocked) {
    ...
    if (lk!.signedSubtotal != null) {
      ...
      subtotal       = Math.round(preTax / (1 - discountPct) * 100) / 100
      discountAmount = Math.round((subtotal - preTax) * 100) / 100
      ...
      const acceptRes = await fetch('/api/accept-estimate', {
        ...
        body: JSON.stringify({
          ...
          ...(estimate.clientContactId ? {
            ...
            itemLabel:    `${applyDiscount ? `${discountPctLabel}% off ` : ''}Cabinet Painting`,
            company: {
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600 mt-0.5">
                ...
                <span className="font-semibold text-green-700">{fmtD(subtotal * discountPct)}</span>{' '}
                off your project.
              </p>
              {applyDiscount && (
