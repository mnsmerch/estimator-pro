...
import { updateInteriorEstimate, deleteInteriorEstimate } from '@/lib/firebase/interiorEstimates'
...
export default function InteriorEstimateDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id }   = use(params)
  ...
  useEffect(() => {
    fetch(`/api/interior-proposal/${id}`)
      .then(r => r.json())
      .then((d: { estimate: InteriorEstimateRecord; company: CompanySettings }) => {
  ...
  async function handleStatusChange(s: EstimateStatus) {
    ...
    try {
      await updateInteriorEstimate(id, { status: s } as Parameters<typeof updateInteriorEstimate>[1])
      setEstimate(prev => prev ? { ...prev, status: s } as InteriorEstimateRecord : prev)
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    try { await deleteInteriorEstimate(id); router.replace('/estimates') }
    catch { alert('Delete failed.'); setDeleting(false) }
  }

  async function handleConvert(toType: 'exterior' | 'cabinet') {
    if (!user || converting) return
    setMoreOpen(false)
    if (!confirm(`Convert this interior estimate to ${toType}? Client info & photos carry over; the interior draft will be replaced.`)) return
    setConverting(true)
    try {
      ...
      const res   = await fetch('/api/convert-estimate-type', {
        ...
        body: JSON.stringify({ estimateId: id, fromType: 'interior', toType }),
      })
  ...
  async function handleEmail() {
    ...
    try {
      await fetch('/api/send-estimate', {
        ...
        body: JSON.stringify({
          clientName: estimate.clientName, clientAddress: estimate.address,
          clientPhone: estimate.clientPhone, clientEmail: estimate.clientEmail,
          clientContactId: estimate.clientContactId ?? '', clientFolderId: '',
          estimateUrl: `${window.location.origin}/ip/${id}`,
          estimateId: id, estimateType: 'interior',
        }),
      })
    ...
    try {
      const res = await fetch('/api/work-orders/create', {
        ...
        body: JSON.stringify({
          estimateId: id, estimateType: 'interior',
          clientName: estimate.clientName, clientAddress: estimate.address ?? '',
          clientEmail: estimate.clientEmail, clientPhone: estimate.clientPhone,
          clientContactId: estimate.clientContactId ?? '',
          scopeOfWork: [estimate.scope?.projectDescription, estimate.scope?.prepWork].filter(Boolean).join('\n\n'),
  ...
  async function handleSaveCO() {
    ...
    try {
      const res  = await fetch('/api/change-order', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ estimateId: id, items: coItems }),
      })
  ...
  if (!estimate) return <div className="flex flex-col items-center justify-center py-32 gap-3"><p className="text-gray-500">Estimate not found.</p><a href="/estimates" className="text-sm text-brand-600">← Back to Estimates</a></div>

  const currentStatus = estimate.status ?? 'draft'
  const proposalUrl   = `/ip/${id}`
  ...
  async function handleRetrigger() {
    ...
    try {
      // Fetch pricing from proposal API
      const proposalRes = await fetch(`/api/interior-proposal/${id}`)
      const proposalData = await proposalRes.json() as { estimate: typeof estimate; rules: { depositPercent?: number; salesDiscount?: number } }
      const rules        = proposalData.rules ?? {}
      const salesDiscount = rules.salesDiscount ?? 0.10
      const depositPct    = rules.depositPercent ?? 0.20
      // We need to get the grandTotal — for now use signedGrandTotal if stored, otherwise stored totalWithTax isn't available
      // Store pricing via accept-estimate so callback can use it
      await fetch('/api/accept-estimate', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          estimateId: id, estimateType: 'interior',
          signatureName: estimate.signatureName ?? '',
          // We can't easily recalculate here — store what we know
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="sticky top-0 z-30 bg-white border-b border-gray-200 px-4 sm:px-6 py-3">
        <div className="max-w-5xl mx-auto flex items-center gap-2 flex-wrap">
          <a href="/estimates" className="flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 mr-2">
          ...
          <h1 className="text-sm font-bold text-gray-900 mr-auto">
            {(estimate as typeof estimate & { estimateNumber?: number }).estimateNumber ? `Estimate #${(estimate as typeof estimate & { estimateNumber?: number }).estimateNumber}` : estimate.clientName}
          </h1>
          <a href={`/estimates/interior/${id}/edit`} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50">
            ...
            {statusOpen && (
              <div className="absolute right-0 mt-1.5 w-36 bg-white rounded-xl border border-gray-200 shadow-lg z-50 py-1">
                {STATUS_OPTIONS.map(opt => (
                  <button key={opt.value} onClick={() => handleStatusChange(opt.value)} className={`w-full text-left px-3 py-2 text-sm font-medium hover:bg-gray-50 ${currentStatus === opt.value ? 'text-brand-700 bg-brand-50' : 'text-gray-700'}`}>{opt.label}</button>
                ))}
              </div>
            )}
          </div>
          <a href={proposalUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-brand-600 text-white text-sm font-semibold hover:bg-brand-700">
          ...
          <div className="relative" ref={moreRef}>
            <button onClick={() => setMoreOpen(v => !v)} className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50">
              More <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" /></svg>
            </button>
            {moreOpen && (
              <div className="absolute right-0 mt-1.5 w-48 bg-white rounded-xl border border-gray-200 shadow-lg z-50 py-1">
                <button onClick={handleCreateWO} disabled={creatingWO || woDone} className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50">
                  {creatingWO ? 'Creating…' : woDone ? '✓ Work Order Created' : 'Create Work Order'}
                </button>
                {estimate.status === 'approved' && (
                  <button onClick={() => { setMoreOpen(false); setShowCO(true); if (!coItems.length) addCoItem() }} className="w-full text-left px-4 py-2.5 text-sm text-amber-700 hover:bg-amber-50 font-medium">
                    Change Order
                  </button>
                )}
                ...
                {currentStatus === 'draft' && (
                  <>
                    ...
                    <button onClick={() => handleConvert('cabinet')} disabled={converting} className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50">{converting ? 'Converting…' : 'Cabinet'}</button>
                  </>
                )}
                <div className="border-t border-gray-100 my-1" />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="text-right shrink-0">
              {(estimate as typeof estimate & { estimateNumber?: number }).estimateNumber && (
                <p className="text-2xl font-bold text-gray-900">#{(estimate as typeof estimate & { estimateNumber?: number }).estimateNumber}</p>
              )}
          ...
          return (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-sm font-semibold text-amber-900">Change Order</p>
              </div>
              <div className="space-y-1.5 text-sm">
                {items.map(item => <div key={item.id} className="flex justify-between text-amber-800"><span>{item.description}</span><span className="tabular-nums">{item.price>=0?'+ ':'− '}{fmtD(Math.abs(item.price))}</span></div>)}
                <div className="border-t border-amber-200 pt-2 flex justify-between font-bold text-gray-900"><span>New Balance Due</span><span className="tabular-nums">{fmtD(newBal)}</span></div>
              </div>
            </div>
          )
        })()}
            ...
            <div className="space-y-2 mb-3">
              {coItems.map((item, idx) => (
                <div key={item.id} className="flex gap-2 items-start">
                  ...
                  <input type="text" value={item.description} onChange={e=>updateCoItem(item.id,'description',e.target.value)} placeholder="Description…" className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" />
                  <div className="relative w-28 shrink-0"><span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-sm">$</span><input type="number" step="0.01" value={item.price||''} onChange={e=>updateCoItem(item.id,'price',parseFloat(e.target.value)||0)} placeholder="0.00" className="w-full border border-gray-300 rounded-lg pl-6 pr-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500" /></div>
                  <button onClick={()=>removeCoItem(item.id)} className="mt-1.5 p-1.5 text-gray-400 hover:text-red-500 rounded-lg hover:bg-red-50"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12"/></svg></button>
                </div>
              ))}
            </div>
            <button onClick={addCoItem} className="flex items-center gap-1.5 text-sm font-medium text-brand-600 hover:text-brand-800 mb-3"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15"/></svg>Add Item</button>
            {coError && <p className="text-xs text-red-600 mb-2">{coError}</p>}
            <div className="flex gap-2">
              <button onClick={handleSaveCO} disabled={coSaving||!coItems.length||!coItems.some(i=>i.description&&i.price!==0)} className="flex-1 py-2 bg-amber-600 text-white text-sm font-semibold rounded-lg hover:bg-amber-700 disabled:opacity-50">{coSaving?'Saving…':'Save Change Order'}</button>
              <button onClick={()=>{setShowCO(false);setCoItems([]);setCoError(null)}} className="px-4 py-2 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50">Cancel</button>
            </div>
          </div>
        )}
        ...
        {estimate.status === 'approved' && estimate.signatureName && (
          <div className="bg-green-50 border border-green-200 rounded-xl p-5">
            <div className="flex items-center gap-3 mb-2">
              <div className="w-7 h-7 bg-green-100 rounded-full flex items-center justify-center shrink-0"><svg className="w-4 h-4 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg></div>
              <div><p className="text-sm font-bold text-green-800">Estimate Accepted</p><p className="text-xs text-green-600">Signed by <strong>{estimate.signatureName}</strong>{estimate.signatureDate ? ` on ${estimate.signatureDate}` : ''}</p></div>
            </div>
            ...
            {/* Re-trigger for old signed estimates with no contactId */}
            {!estimate.clientContactId && (
              <div className="mt-4 pt-4 border-t border-green-200">
                {retriggerDone ? (
                  <p className="text-sm font-semibold text-green-700">✓ Sent to GHL — invoices will be created shortly.</p>
                ) : retriggerError ? (
                  <p className="text-sm text-red-600">{retriggerError}</p>
                ) : (
                  <button onClick={handleRetrigger} disabled={retrigger} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-green-700 text-white text-sm font-semibold hover:bg-green-800 disabled:opacity-50">
        ...
        {/* Scope */}
        {(estimate.scope?.projectDescription || estimate.scope?.prepWork) && (
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h3 className="text-base font-semibold text-gray-900 mb-4">Scope of Work</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm">
              {estimate.scope.projectDescription && <div><p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-1">Project</p><p className="whitespace-pre-line text-gray-700">{estimate.scope.projectDescription}</p></div>}
              {estimate.scope.prepWork && <div><p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-1">Prep Work</p><p className="whitespace-pre-line text-gray-700">{estimate.scope.prepWork}</p></div>}
            </div>
          </div>
        )}
