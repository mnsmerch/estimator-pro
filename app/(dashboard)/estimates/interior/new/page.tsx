...
import { createInteriorEstimate } from '@/lib/firebase/interiorEstimates'
...
function newWallMeasurement(): WallMeasurement {
  return { id: uid(), length: '', height: '' }
}

function newWallSection(): WallSection {
  return { id: uid(), wallType: '', measurements: [newWallMeasurement()] }
}

function newCeilingMeasurement(): CeilingMeasurement {
  return { id: uid(), length: '', width: '' }
}

function newCeilingSection(): CeilingSection {
  return { id: uid(), ceilingType: '', measurements: [newCeilingMeasurement()] }
}

function newBaseboardMeasurement(): BaseboardMeasurement {
  return { id: uid(), length: '' }
}

function newBaseboardSection(): BaseboardSection {
  return { id: uid(), baseboardType: '', measurements: [newBaseboardMeasurement()] }
}

function newDoorEntry(): DoorEntry {
  return { id: uid(), doorType: '', count: '' }
}

function newDoorFrameEntry(): DoorFrameEntry {
  return { id: uid(), doorFrameType: '', count: '' }
}

function newWindowEntry(): WindowEntry {
  return { id: uid(), windowType: '', count: '' }
}

function newOtherEntry(): OtherEntry {
...
function newOption(name = ''): RoomOption {
  return {
    ...
    walls:      [newWallSection()],
    ceilings:   [newCeilingSection()],
...
export default function NewInteriorEstimatePage() {
  const router = useRouter()

  const initialOption = newOption('Room 1')
  const [draft, setDraft] = useState<InteriorEstimateDraft>({
    clientName: '',
    address:    '',
    options:    [initialOption],
  })
  const [activeId, setActiveId] = useState<string>(initialOption.id)
  ...
  function patchOption(optId: string, patch: Partial<RoomOption>) {
    setDraft(prev => ({
      ...prev,
      options: prev.options.map(o => o.id === optId ? { ...o, ...patch } : o),
    }))
  }
  ...
  function patchPaint(optId: string, key: keyof PaintSelections, value: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id === optId ? { ...o, paints: { ...o.paints, [key]: value } } : o
      ),
    }))
  }
  ...
  function addWallSection(optId: string) {
    setDraft(prev => ({
      ...prev,
      options: prev.options.map(o =>
        o.id !== optId ? o : { ...o, walls: [...o.walls, newWallSection()] }
      ),
    }))
  }
  ...
  function removeWallSection(optId: string, sid: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, walls: o.walls.length > 1 ? o.walls.filter(s => s.id !== sid) : o.walls,
        }
      ),
    }))
  }
  ...
  function setWallType(optId: string, sid: string, wallType: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, walls: o.walls.map(s => s.id !== sid ? s : { ...s, wallType }),
        }
      ),
    }))
  }
  ...
  function addWallRow(optId: string, sid: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, walls: o.walls.map(s =>
            s.id !== sid ? s : { ...s, measurements: [...s.measurements, newWallMeasurement()] }
          ),
        }
      ),
    }))
    ...
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, walls: o.walls.map(s =>
            s.id !== sid ? s : {
              ...s, measurements: s.measurements.length > 1
                ? s.measurements.filter(m => m.id !== mid)
                : s.measurements,
            }
          ),
        }
    ...
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, walls: o.walls.map(s =>
            s.id !== sid ? s : {
              ...s, measurements: s.measurements.map(m =>
                m.id !== mid ? m : { ...m, [field]: value }
              ),
            }
          ),
        }
  ...
  function addCeilingSection(optId: string) {
    setDraft(prev => ({
      ...prev,
      options: prev.options.map(o =>
        o.id !== optId ? o : { ...o, ceilings: [...o.ceilings, newCeilingSection()] }
      ),
    }))
  }
  ...
  function removeCeilingSection(optId: string, sid: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, ceilings: o.ceilings.length > 1 ? o.ceilings.filter(s => s.id !== sid) : o.ceilings,
        }
      ),
    }))
  }
  ...
  function setCeilingType(optId: string, sid: string, ceilingType: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, ceilings: o.ceilings.map(s => s.id !== sid ? s : { ...s, ceilingType }),
        }
      ),
    }))
  }
  ...
  function addCeilingRow(optId: string, sid: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, ceilings: o.ceilings.map(s =>
            s.id !== sid ? s : { ...s, measurements: [...s.measurements, newCeilingMeasurement()] }
          ),
        }
      ),
    }))
  ...
  function addBaseboardSection(optId: string) {
    setDraft(prev => ({
      ...prev,
      options: prev.options.map(o =>
        o.id !== optId ? o : { ...o, baseboards: [...o.baseboards, newBaseboardSection()] }
      ),
    }))
  }
  ...
  function removeBaseboardSection(optId: string, sid: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, baseboards: o.baseboards.length > 1 ? o.baseboards.filter(s => s.id !== sid) : o.baseboards,
        }
      ),
    }))
  }
  ...
  function setBaseboardType(optId: string, sid: string, baseboardType: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, baseboards: o.baseboards.map(s => s.id !== sid ? s : { ...s, baseboardType }),
        }
      ),
    }))
  }
  ...
  function addBaseboardRow(optId: string, sid: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, baseboards: o.baseboards.map(s =>
            s.id !== sid ? s : { ...s, measurements: [...s.measurements, newBaseboardMeasurement()] }
          ),
        }
      ),
    }))
  ...
  function addDoorEntry(optId: string) {
    setDraft(prev => ({
      ...prev,
      options: prev.options.map(o =>
        o.id !== optId || o.doors.length >= MAX_DOOR_TYPES ? o
          : { ...o, doors: [...o.doors, newDoorEntry()] }
      ),
    }))
  }
  ...
  function removeDoorEntry(optId: string, eid: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, doors: o.doors.length > 1 ? o.doors.filter(d => d.id !== eid) : o.doors,
        }
      ),
    }))
  }
  ...
  function patchDoorEntry(optId: string, eid: string, field: keyof DoorEntry, value: string | number | '') {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, doors: o.doors.map(d => d.id !== eid ? d : { ...d, [field]: value }),
        }
      ),
    }))
  }
  ...
  function addDoorFrameEntry(optId: string) {
    setDraft(prev => ({
      ...prev,
      options: prev.options.map(o =>
        o.id !== optId || o.doorFrames.length >= MAX_DOOR_FRAME_TYPES ? o
          : { ...o, doorFrames: [...o.doorFrames, newDoorFrameEntry()] }
      ),
    }))
  }
  ...
  function removeDoorFrameEntry(optId: string, eid: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, doorFrames: o.doorFrames.length > 1 ? o.doorFrames.filter(d => d.id !== eid) : o.doorFrames,
        }
      ),
    }))
  }
  ...
  function patchDoorFrameEntry(optId: string, eid: string, field: keyof DoorFrameEntry, value: string | number | '') {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, doorFrames: o.doorFrames.map(d => d.id !== eid ? d : { ...d, [field]: value }),
        }
      ),
    }))
  }
  ...
  function addWindowEntry(optId: string) {
    setDraft(prev => ({
      ...prev,
      options: prev.options.map(o =>
        o.id !== optId || o.windows.length >= MAX_WINDOW_TYPES ? o
          : { ...o, windows: [...o.windows, newWindowEntry()] }
      ),
    }))
  }
  ...
  function removeWindowEntry(optId: string, eid: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, windows: o.windows.length > 1 ? o.windows.filter(w => w.id !== eid) : o.windows,
        }
      ),
    }))
  }
  ...
  function patchWindowEntry(optId: string, eid: string, field: keyof WindowEntry, value: string | number | '') {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, windows: o.windows.map(w => w.id !== eid ? w : { ...w, [field]: value }),
        }
      ),
    }))
  }
  ...
  function addOtherEntry(optId: string) {
    setDraft(prev => ({
      ...prev,
      options: prev.options.map(o =>
        o.id !== optId ? o : { ...o, otherEntries: [...o.otherEntries, newOtherEntry()] }
      ),
    }))
  }
  ...
  function removeOtherEntry(optId: string, eid: string) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, otherEntries: o.otherEntries.length > 1
            ? o.otherEntries.filter(e => e.id !== eid)
            : o.otherEntries,
        }
      ),
    }))
  ...
  function patchOtherEntry(optId: string, eid: string, patch: Partial<OtherEntry>) {
    setDraft(prev => ({
      ...
      options: prev.options.map(o =>
        o.id !== optId ? o : {
          ...o, otherEntries: o.otherEntries.map(e => e.id !== eid ? e : { ...e, ...patch }),
        }
      ),
    }))
  }
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      await createInteriorEstimate(draft)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-4">New Interior Estimate</h1>
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            {activeOption.walls.map((section, si) => (
              <MeasurementCard
                ...
                label="Wall Type"
                index={si}
                selectedType={section.wallType}
                typeOptions={WALL_TYPE_OPTIONS}
                typePlaceholder="Select wall type…"
                col1Label="Length (ft)"
                col2Label="Height (ft)"
            ...
            {activeOption.ceilings.map((section, si) => (
              <MeasurementCard
                ...
                label="Ceiling Type"
                index={si}
                selectedType={section.ceilingType}
                typeOptions={CEILING_TYPE_OPTIONS}
                typePlaceholder="Select ceiling type…"
                col1Label="Length (ft)"
                col2Label="Width (ft)"
            ...
            {activeOption.baseboards.map((section, si) => (
              <LengthOnlyCard
                ...
                index={si}
                selectedType={section.baseboardType}
                typeOptions={BASEBOARD_TYPE_OPTIONS}
                typePlaceholder="Select baseboard type…"
            ...
            <CountCard
              ...
              typePlaceholder="Select door type…"
            ...
            <CountCard
              label="Door Frame Types"
              typeOptions={DOOR_FRAME_TYPE_OPTIONS}
              typePlaceholder="Select door frame type…"
            ...
            <CountCard
              label="Window Types"
              typeOptions={WINDOW_TYPE_OPTIONS}
              typePlaceholder="Select window type…"
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="px-5 py-3 space-y-3">
                ...
                {activeOption.otherEntries.map(entry => (
                  <div key={entry.id} className="grid grid-cols-[1fr_120px_120px_auto] gap-3 items-center">
                    ...
                    <input
                      ...
                      onChange={e => {
                        const raw = e.target.value
                        patchOtherEntry(activeOption.id, entry.id, { hours: raw === '' ? '' : parseFloat(raw) || 0 })
                      }}
                    ...
                    <input
                      ...
                      onChange={e => {
                        const raw = e.target.value
                        patchOtherEntry(activeOption.id, entry.id, { gallons: raw === '' ? '' : parseFloat(raw) || 0 })
                      }}
                      className="px-3 py-2 border border-gray-300 rounded-lg text-sm text-center focus:outline-none focus:ring-2 focus:ring-brand-500"
                    />
                    <button
                ...
                <button
                  onClick={() => addOtherEntry(activeOption.id)}
...
// Single-value measurement card — for sections that need one input per row (e.g. baseboard length)
function LengthOnlyCard({
  label, index, selectedType, typeOptions, typePlaceholder,
  measurements, canDelete,
  ...
  measurements:    { id: string; value: number | '' }[]
  canDelete:       boolean
  onTypeChange:    (v: string) => void
  onDelete:        () => void
  onAddRow:        () => void
  onRemoveRow:     (id: string) => void
  onPatchRow:      (id: string, val: string) => void
}) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      <div className="px-5 py-3 space-y-2">
        ...
        {measurements.map(m => (
          <div key={m.id} className="grid grid-cols-[200px_auto] gap-3 items-center">
            <input
              type="number" step="0.1" min="0" placeholder="0"
              value={m.value}
              onChange={e => onPatchRow(m.id, e.target.value)}
              className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
            />
            <button
        ...
        <button
          onClick={onAddRow}
          className="flex items-center gap-1.5 text-xs text-brand-600 hover:text-brand-700 font-medium mt-2 transition-colors"
        >
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
          </svg>
...
// Generic count card — shared by doors, door frames, and future count-only sections
function CountCard({
  ...
  typeOptions:      { key: string; label: string }[]
  typePlaceholder:  string
  entries:          { id: string; type: string; count: number | '' }[]
  max:              number
  onAdd:            () => void
        ...
        {entries.map(entry => (
          <div key={entry.id} className="grid grid-cols-[1fr_140px_auto] gap-3 items-center">
            ...
            <input
              ...
              onChange={e => {
                const raw = e.target.value
                onCountChange(entry.id, raw === '' ? '' : parseInt(raw) || 0)
              }}
              className="px-3 py-2 border border-gray-300 rounded-lg text-sm text-center focus:outline-none focus:ring-2 focus:ring-brand-500"
            />
            <button
...
// Generic measurement card — shared by walls, ceilings, and future sections
function MeasurementCard({
  ...
  col1Label:        string
  col2Label:        string
  measurements:     { id: string; a: number | ''; b: number | '' }[]
  canDelete:        boolean
  onTypeChange:     (v: string) => void
  onDelete:         () => void
  onAddRow:         () => void
        ...
        {measurements.map(m => (
          <div key={m.id} className="grid grid-cols-[1fr_1fr_auto] gap-3 items-center">
            <input
              type="number" step="0.1" min="0" placeholder="0"
              value={m.a}
              onChange={e => onPatchRow(m.id, 'a', e.target.value)}
              className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
            />
            <input
              type="number" step="0.1" min="0" placeholder="0"
              value={m.b}
              onChange={e => onPatchRow(m.id, 'b', e.target.value)}
              className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
            />
            <button
              onClick={() => onRemoveRow(m.id)}
